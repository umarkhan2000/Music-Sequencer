...
BST_Node *newBST_Node(double freq, int bar, double index)
{
     ...
     ****/
    BST_Node *new_Bnode = NULL;
	
	new_Bnode = (BST_Node *)calloc(1, sizeof(BST_Node));
	
	new_Bnode->key = (10.0*bar)+index;
	new_Bnode->freq = freq;
	new_Bnode->bar = bar;
	new_Bnode->index = index;
	new_Bnode->left = NULL;
	new_Bnode->right = NULL;
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
	...
	if(new_node->key < root->key && new_node->key != root->key)
	{
		root->left = BST_insert(root->left, new_node);
	}
	else if(new_node->key > root->key && new_node->key != root->key)
	{
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
	...
	double key_wanted = (10.0*bar)+index; 
	
	if(root == NULL) return NULL;
	
	if(root->key == key_wanted)
	{
		return root;
	}
	else if(root->key > key_wanted)
	{
...
BST_Node *find_successor(BST_Node *right_child_node)
{
	...
	{
		right_child_node = find_successor(right_child_node->left);
	}		
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
     ...
     ****/
	  BST_Node *t = NULL;
	 double key_given = (double)(10.0*bar)+index; 
	 ...
	 if(root->key == key_given)
	 {
		 if(root->left == NULL && root->right == NULL)
		 {
			 free(root);								// first case
			 return NULL;
		 }
...
void BST_makePlayList(BST_Node *root)
{
	...
	playlist_head=playlist_insert(playlist_head, c_freq, c_bar, c_index);
...
void BST_inOrder(BST_Node *root, int depth)
{
     ...
     ****/
	 ...
	 if (root != NULL){
		int n_bar = root->bar;
		double n_index = root->index;
		double n_freq = root->freq;
		 
		if (root->left != NULL){
			BST_inOrder(root->left, depth + 1);
		}
	 
		printf("Depth=%d, Bar:Index (%d:%f), F=%f Hz\n", depth, n_bar, n_index, n_freq);
...
void BST_shiftFreq(BST_Node *root, char note_src[5], char note_dst[5])
{
     ...
     ****/
	 ...
	 for(k=0; k<100; k++){
		if (strcmp(note_names[k], note_src) == 0){
			src_freq = note_freq[k];
		}
		 
		if (strcmp(note_names[k], note_dst) == 0){
			dst_freq = note_freq[k];
		}
	 }
 ...
 * ******************************************************************************************/
 BST_Node *new_tree(BST_Node *root, int semitones, double time_shift, BST_Node *t_root)
 {
	 if(root == NULL) return NULL;
	 ...
	 for(j = 0; j<100; j++)
	 {
		if(note_freq[j] == root->freq) break;
	 }
	 
	 
	 if(j + semitones <0 || j + semitones > 99 || root->index + time_shift < 0||root->index+time_shift >= 1)
	 {	 
		return NULL;
	 }
	
	 
	 BST_Node *new_node = newBST_Node(note_freq[j +semitones], root->bar, root->index + time_shift);
	 
	 t_root = BST_insert(t_root, new_node);
	 
	 new_tree(root->left, semitones, time_shift, t_root);
	 new_tree(root->right, semitones, time_shift, t_root);
	 
	 return t_root;
 }
 
 void insert(BST_Node *root, BST_Node *t_root)
 {
	 if(t_root == NULL) return;
	 
	 insert(root, t_root->left);
	 insert(root, t_root->right);
	 
	 t_root->left = NULL;
	 t_root->right = NULL;
	 
	 BST_insert(root, t_root);
 }
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     ****/
	 ...
	 BST_Node *t_root = NULL;
	 t_root = new_tree(root, semitones, time_shift, t_root);
	 insert(root, t_root);
	 return root;
